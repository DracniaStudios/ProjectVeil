...
void Crane::update(Scene* scene_ptr, float deltaTime)
{
	...
	for (auto obstacle : data->obstacles)
	{
		if (CheckCollisionCircleRec(player->getPosition2D(), player->rigidBody2D.scale.x, obstacle))
		{
			...
			// obstacles vector) before building a replacement, so continuing to
			// iterate `data->obstacles` or read `data` below is a use-after-free.
			return;
		}
	}
	...
	/// Goal Logic
	{
		if (CheckCollisionCircleRec(player->rigidBody2D.getPosition(), player->rigidBody2D.scale.x, data->goal))
		{
			...
			// ReleaseMiniGame() deleted `data` and nulled scene_ptr->miniGame, so
			// nothing below may read either one.
			return;
		}
